...
jQuery(function ($) { // DOM ready
    $('.iframeWrapper').on('click', function (e) {
        ...
        image_tag = '<img id="preview_img2" class="doc custom_img_border" src="' + iframe_src + '" frameborder="0" style="width:100%;" height="500px"></img>';
        $("#div_preview_content_filepdf").html(image_tag);
        show_pdf_content_data(pdf_content_id);
        $("#show_loader").show();
        scroll_content();
        var img_obj = { img: iframe_src };

        $.ajax({
            url: "/Ajax/ImageProcessingModel/",
            type: "POST",
            data: img_obj,
               ...
               // return false;
                ...
                if (data.StatusCode == 200) {
                    ...
                    if (pdf_content_id == 1) {
                        ...
                        for (var i = 0; i < data.Tag_list_color.length; i++) {
                            console.log(data.Tag_list_color[i]);
                            var color_index = i + 1;
                            newHTML += '<p class="line_data_align">' + color_index + ':<span class="tx - gray - 800" id="invoice_no"> ' + data.Tag_list_color[i]+'</span></p>'
                        }
                        $("#data_color_content").html(newHTML);
                    }
