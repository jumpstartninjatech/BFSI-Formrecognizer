...
namespace vChek_VFS
{
    class Program
    {
        ...
        static string BlobStorageContainerName = "unprocessed";
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                foreach (PathItem pathItem in filesystem.GetPaths())
                {
                    ...
                    // check file exists
                    string checkDataExists = DataHandler.InsertDocumentExtractionDetails(filename);
                    if (checkDataExists == "100")
                    {
                        Console.WriteLine("*****************************************************");
                        Console.WriteLine("The file name " + pathItem.Name + " already processed from the data lake....");
                        Console.WriteLine("*****************************************************");
                    }
                    else
                    {
